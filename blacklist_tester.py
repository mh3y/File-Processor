...
import json
import argparse
from tools_static import Github
...
def create_github_issue_for_blacklist(sql_files_json, header, user_name, repo_name):
    ...
    for files in sql_files_json:
        ...
        github_object.create_github_issue(user_name, header, issue)
...
def main():
    ...
    if args.rename:
        ...
        pretty_print(sql_files_json)
    if args.gituser:
        github.get_github_user(header=h, user_name=args.gituser)
    # if args.postissue is True:
    #     create_github_issue(h, args.gituser, test_issue)
    if args.testpostissue is True:
        github.create_github_issue(user_name=args.gituser, header=h, new_issue=test_issue)
    if args.listissues is True:
        github.list_github_issues(user_name=args.gituser, header=h, repo_name=github_repo_name)
